    ...
    public static String interpretURL(String in) {
        ...
        if (matcher.find()) {
            try {
                ...
                ReportWriter.writeToHtmlReport("Unable to supply a default scheme, or host");
            }
        }
        // if we have got here, there has been a problem (e.g. unable to match, or get defaults (if needed))
        ReportWriter.writeToHtmlReport("Unable to make a good URL out of >" + in + "<");
        ReportWriter.writeToHtmlReport("scheme: " + matcher.group(1));
        ReportWriter.writeToHtmlReport("scheme's magic letters: " + matcher.group(2));
        ReportWriter.writeToHtmlReport("host: " + matcher.group(3));
        ReportWriter.writeToHtmlReport("the rest: " + matcher.group(4));
        return null;
    }
